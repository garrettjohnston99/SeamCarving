...
{
	if (i < 0 || i >= Array3D::size()) throw OutOfBoundsException();
...
{
	if (x < 0 || x >= Array3D::width() ||
		y < 0 || y >= Array3D::height() ||
		z < 0 || z >= Array3D::depth()) throw OutOfBoundsException();

	// Flattening formula from 9/15 x-hour 
	int index = z * Array3D::width() * Array3D::height() + y * Array3D::width() + x;
	return m_data[index];
}
