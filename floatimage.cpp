...
{
	if (clampToEdge) {
		// Clamp each dimension and return the pixel value there
		return operator()(clamp(x, 0, this->width() - 1), 
						  clamp(y, 0, this->height() - 1), 
						  clamp(z, 0, this->channels() - 1));
	} 

	// Not clamping to edge
	// Parameter out of bounds, return black pixel
	if (x < 0 || x > this->width() - 1 ||
		y < 0 || y > this->height() - 1 ||
		z < 0 || z > this->channels() - 1) {
		return 0.0f;
	}

	// Parameters are good, return the pixel value at this position
