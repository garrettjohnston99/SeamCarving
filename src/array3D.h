...
template <typename T>
inline Array3D<T>::Array3D() : m_data(), m_sizeX(0), m_sizeY(0), m_sizeZ(0), m_strideZ(0)
{
	// empty
}

template <typename T>
inline Array3D<T>::Array3D(int sizeX, int sizeY, int sizeZ) : m_data(sizeX * sizeY * sizeZ),
...
inline T &Array3D<T>::operator()(int x, int y, int z)
{
	if (
			...
			(z < Array3D::sizeZ()))
	{
		return m_data[z * Array3D::width() * Array3D::height() + y * Array3D::width() + x];
	}
	else
	{
		throw OutOfBoundsException();
